...
incrementer_proc_of.each_pair do |delta, incrementer_proc| #each pair method
  (0..5).to_a.each do |other_addend|
    puts "#{delta} + #{other_addend} =  #{ incrementer_proc.call(other_addend)} "
    end
  end
